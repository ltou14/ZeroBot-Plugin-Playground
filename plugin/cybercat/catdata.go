...
// AddCatFood 增加指定用户的猫粮数量（单位：斤）
// @param userID 用户ID
// @param amount 增加的猫粮数量
// @return 错误信息
func AddCatFood(userID int64, amount float64) error {
	gidStr := "catinfo"
	uidStr := strconv.FormatInt(userID, 10)

	catdata.Lock()
	defer catdata.Unlock()

	// 查找用户猫信息
	var info catInfo
	err := catdata.Find(gidStr, &info, "WHERE user = "+uidStr)
	if err != nil {
		return err
	}
	if info.Name == "" { // 无猫
		return errors.New("该用户没有猫")
	}
	...
	// 保存更新
	return catdata.Insert(gidStr, &info)
}
